"""FastAPI application for gpu-cluster-scheduler."""
...
app = FastAPI(title="gpu-cluster-scheduler", version="0.1.0", description="gpu-cluster-scheduler API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "gpu-cluster-scheduler", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "gpu-cluster-scheduler", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "gpu-cluster-scheduler", "version": "0.1.0", "ready": True,
